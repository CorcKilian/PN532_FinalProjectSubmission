...
void init_display()
{
    ...
    //initSPI(SPI1);
    delay_ms(100);
    command(0x01); // Dummy Bit to clear
...
void fillRectangle(uint16_t x,uint16_t y,uint16_t width, uint16_t height, uint16_t colour)
{
	...
	while(pixelcount--) 
	{
        colour = Reverse(colour,16);
		transferSPI16(SPI1,colour);
	}	
}
int Reverse(int value, int shift) {
    __asm volatile (
        "rbit %0, %0\n\t"   // Reverse the bits in 'value'
        "lsr  %0, %0, %1"   // Logical shift right by 'shift' bits
        : "+r" (value)      // 'value' is both an input and an output operand
        : "r"  (shift)      // 'shift' is a pure input operand
    );
    return value;
}
void putPixel(uint16_t x, uint16_t y, uint16_t colour)
...
// Need to toggle the CS to allow for Multiple SPI devices.
